...
class GreatCircleBinner(object):
    def __init__(
        self, min_lat, max_lat, lat_count, min_lng, max_lng, lng_count
    ):
        ...
        self.bins = np.zeros(
            (self.lngs.count, self.lats.count), dtype="uint32"
        )
    ...
    def add_point(self, point):
        ...
        if not (self.lngs.min <= point.lng <= self.lngs.max) or not (
            self.lats.min <= point.lat <= self.lats.max
        ):
            return

        lng_index = int(
            round(
                ((point.lng - self.lngs.min) / self.lngs.range)
                * (self.lngs.count - 1)
            )
        )
        lat_index = int(
            round(
                ((point.lat - self.lats.min) / self.lats.range)
                * (self.lats.count - 1)
            )
        )
    ...
    def add_greatcircle(self, point_1, point_2, max_npts=3000):
        for point in greatcircle_points(
            point_1, point_2, self.max_range, max_npts
        ):
